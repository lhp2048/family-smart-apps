...
import 'package:pdfx/src/renderer/io/platform_web_open_stub.dart'
    if (dart.library.html) 'package:pdfx/src/renderer/io/platform_web_open_web.dart';
...
class PdfxPlatformMethodChannel extends PdfxPlatform {
  ...
  Future<PdfDocument> openFile(String filePath, {String? password}) async {
    if (UniversalPlatform.isWeb) {
      final opened = await openPdfFileOnWeb(filePath, password: password);
      if (opened != null) {
        return opened;
      }
  ...
  Future<PdfDocument> openData(FutureOr<Uint8List> data,
    ...
    if (UniversalPlatform.isWeb) {
      final opened = await openPdfDataOnWeb(data, password: password);
      if (opened != null) {
        return opened;
      }
      throw PlatformNotSupportedException();
