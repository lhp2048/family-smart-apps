...
import 'package:pdfx/src/renderer/web/url_load_progress.dart';
...
@JS('familyPdfJsGetDocumentUrl')
external JSPromise<PdfjsDocument> _familyPdfJsGetDocumentUrl(
  String url,
  String? password,
  JSFunction? onProgressFn,
);
...
Future<PdfjsDocument> pdfjsGetDocument(
  ...
  final sessionId = PdfjsUrlLoadProgressScope.consumeHandoffSessionId();
  JSFunction? onProgressFn;
  if (sessionId >= 0) {
    onProgressFn = ((JSNumber loaded, JSNumber total) {
      PdfjsUrlLoadProgressScope.dispatch(
        sessionId,
        loaded.toDartInt,
        total.toDartInt,
      );
  ...
  return await _familyPdfJsGetDocumentUrl(
    url,
    password,
    onProgressFn,
  ).toDart;
