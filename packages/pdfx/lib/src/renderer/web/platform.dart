...
class PdfxWeb extends PdfxPlatform {
  ...
  Future<DocumentInfo> _openDocumentUrl(String url, {String? password}) async {
    final document = await pdfjsGetDocument(url, password: password);
    return _documents.register(document).info;
  }

  @override
  Future<PdfDocument> openFile(String filePath, {String? password}) async {
    final obj = await _openDocumentUrl(filePath, password: password);
    return _open(obj, 'url:$filePath');
  }
}
